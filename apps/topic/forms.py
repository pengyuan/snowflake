...
class TopicForm(forms.ModelForm):
    ...
    content = forms.CharField(label=u'内容',widget=PagedownWidget(attrs={'class':'span12 resizable','style':'opacity: 1; height: 180px;'}))   
    class Meta:
        model = Topic
        fields = ('title','content')
        
class ReplyForm(forms.ModelForm):
    content = forms.CharField(widget=PagedownWidget(attrs={'class':'span12 resizable','style':'opacity: 1; height: 180px;'})) 
    class Meta:
        model = Reply
        fields = ('content',)
...
class MessageReply(forms.Form):
    content = forms.CharField(label=u'回复',max_length=200,widget=forms.Textarea(attrs={'id':'message_reply','class':'span12','style':'height:100px;','onkeyup':'words_deal("#message_reply",196);'}))
